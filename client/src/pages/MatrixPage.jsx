...
function heatColor(count, max) {
  ...
  // Indigo-based heatmap (MinMax primary)
  const r = Math.round(238 + (79 - 238) * ratio)
  const g = Math.round(242 + (70 - 242) * ratio)
  const b = Math.round(255 + (229 - 255) * ratio)
  const a = 0.2 + ratio * 0.65
  return `rgba(${r}, ${g}, ${b}, ${a})`
}
...
export default function MatrixPage() {
    ...
    return (
      <div className="min-h-screen bg-[var(--color-surface-dim)] flex items-center justify-center">
        <div className="text-center">
          <Grid3X3 size={48} className="mx-auto mb-4 text-[var(--color-success)] animate-pulse" />
  ...
  return (
    <div className="min-h-screen bg-[var(--color-surface-dim)]">
      {/* 헤더 */}
      <header className="bg-[var(--color-surface)]/80 backdrop-blur-md border-b border-[var(--color-border)] sticky top-0 z-30 print:hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-[var(--color-success)] to-[var(--color-info)] flex items-center justify-center shrink-0">
              <Grid3X3 size={16} className="text-white" />
            </div>
            <h1 className="text-lg font-bold text-[var(--color-text-primary)] truncate">교과 연계 매트릭스</h1>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={handleExportCSV} className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-text-secondary)] rounded-lg hover:bg-[var(--color-surface-bright)] transition">
              <Download size={14} /> CSV
            </button>
            <button onClick={() => window.print()} className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-text-secondary)] rounded-lg hover:bg-[var(--color-surface-bright)] transition">
              <Printer size={14} /> 인쇄
            </button>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1">
              <button onClick={() => setFilterSchoolLevel('')}
                className={`px-2.5 py-1 text-xs rounded-full transition ${!filterSchoolLevel ? 'bg-[var(--color-text-primary)] text-white' : 'bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-bright)]'}`}>전체</button>
              {schoolLevels.map(level => (
                <button key={level} onClick={() => setFilterSchoolLevel(filterSchoolLevel === level ? '' : level)}
                  className={`px-2.5 py-1 text-xs rounded-full transition ${filterSchoolLevel === level ? 'bg-[var(--color-primary)] text-white' : 'bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-bright)]'}`}>{level}</button>
              ))}
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1">
              <button onClick={() => setFilterLinkType('')}
                className={`px-2.5 py-1 text-xs rounded-full transition ${!filterLinkType ? 'bg-[var(--color-text-primary)] text-white' : 'bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-bright)]'}`}>전체</button>
              {Object.entries(LINK_TYPE_LABELS).map(([type, label]) => (
                <button key={type} onClick={() => setFilterLinkType(filterLinkType === type ? '' : type)}
                  className={`px-2.5 py-1 text-xs rounded-full transition ${filterLinkType === type ? 'text-white' : 'bg-[var(--color-surface)] border border-[var(--color-border)] hover:bg-[var(--color-surface-bright)]'}`}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {[
            ...
            { value: linkTypeStats.length, label: '연결 유형', color: 'var(--color-primary-light)' },
          ].map(item => (
            <div key={item.label} className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4 text-center shadow-sm">
        ...
        {/* 매트릭스 테이블 */}
        <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] shadow-sm overflow-hidden mb-6">
              ...
              <tbody>
                {subjectGroups.map(row => (
                  <tr key={row} className="hover:bg-[var(--color-surface-dim)]/50">
                    <td className="sticky left-0 z-10 bg-[var(--color-surface)] border-r border-b border-[var(--color-border)] px-3 py-2 font-semibold whitespace-nowrap" style={{ color: SUBJECT_COLORS[row] || '#6b7280' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          ...
          <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-5 shadow-sm">
            <h3 className="text-sm font-semibold text-[var(--color-text-secondary)] mb-3">연결 유형 분포</h3>
              ...
              {linkTypeStats.map(([type, count]) => {
                ...
                return (
                  <div key={type} className="flex items-center gap-2">
                    ...
                    <div className="flex-1 h-3 bg-[var(--color-surface-dim)] rounded-full overflow-hidden">
          ...
          {/* 교과별 교차 연결 수 */}
          <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-5 shadow-sm">
            <h3 className="text-sm font-semibold text-[var(--color-text-secondary)] mb-3">교과별 교차 연결 수</h3>
            <div className="space-y-2.5">
              {(() => {
        ...
        {selectedCell && (
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 print:hidden" onClick={() => setSelectedCell(null)}>
            <div className="bg-[var(--color-surface)] rounded-2xl shadow-2xl max-w-lg w-full mx-4 max-h-[80vh] overflow-hidden" onClick={e => e.stopPropagation()}>
              ...
              <div className="overflow-auto max-h-[60vh] p-4 space-y-2">
                {selectedCell.links.slice(0, 30).map(({ link, s1, s2 }, i) => (
                  <button key={i} onClick={() => navigate(`/explorer?code=${s1.code}`)}
                    className="w-full text-left p-3 rounded-xl border border-[var(--color-border)] hover:bg-indigo-50 hover:border-[var(--color-primary-light)] transition group">
